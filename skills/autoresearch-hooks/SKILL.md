...
Optional scripts that run at iteration boundaries in an autoresearch session. Two hooks, both transparent to the loop-running agent — their effect is a file on disk or guidance included in the autoresearch tool result.
...
- **Stdout** (up to 8 KB) — included as hook guidance in the autoresearch tool result. Empty = silent.
- **Stderr + non-zero exit** — surfaced as hook error guidance.
...
- **Silent is the default.** Only print to stdout when you have something useful for the agent. Empty stdout means no hook guidance.
